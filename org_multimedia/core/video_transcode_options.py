...
_WEBM_HW_ENCODER: str | None | False = False
...
def reset_hw_encoder_cache() -> None:
    global _HW_ENCODER, _HW_ENCODERS_TEXT, _WEBM_HW_ENCODER
    with _HW_LOCK:
        _HW_ENCODER = False
        _HW_ENCODERS_TEXT = False
        _WEBM_HW_ENCODER = False


def disable_webm_hw_encoder() -> None:
    """Marca el códec WebM HW como no usable (p. ej. VAAPI sin perfil VP8)."""
    global _WEBM_HW_ENCODER
    with _HW_LOCK:
        _WEBM_HW_ENCODER = None
...
def detect_webm_hw_encoder(ffmpeg: str) -> str | None:
    global _WEBM_HW_ENCODER
    with _HW_LOCK:
        if _WEBM_HW_ENCODER is not False:
            return _WEBM_HW_ENCODER
    enc: str | None = None
    if get_transcode_hw_mode() != "off":
    ...
    with _HW_LOCK:
        if _WEBM_HW_ENCODER is not False:
            return _WEBM_HW_ENCODER
        _WEBM_HW_ENCODER = enc
        return enc


def build_webm_encode_options(ffmpeg: str, *, force_cpu: bool = False) -> tuple[str, list[str]]:
